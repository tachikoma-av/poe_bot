...
class Mover:
  ...
  def enterTransition(self, transition: Entity, necropolis_ui=False, screen_pos_offset=[0, 0], entered_distance_sign=100, check_for_loading=False):
    ...
    while entered is False:
      ...
      distance_to_player = dist(
        (int(poe_bot.game_data.player.grid_pos.y), int(poe_bot.game_data.player.grid_pos.x)), (transition.grid_position.y, transition.grid_position.x)
      )
  ...
  def openNearestDoor(self, distance_to_door=15):
    ...
    # ignore the fake one
    doors_nearby = list(filter(lambda entity: entity.path and "ScepterDoorLightSpawned" not in entity.path, doors_nearby))
    if len(doors_nearby) > 0:
  ...
  def goToPoint(
    ...
    path = []
    path = poe_bot.pather.generatePath(
      (int(poe_bot.game_data.player.grid_pos.y), int(poe_bot.game_data.player.grid_pos.x)), (grid_pos_to_go_y, grid_pos_to_go_x)
    )
